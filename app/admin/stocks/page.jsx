...
export default function AdminStocksPage() {
  const [rawPurchases, setRawPurchases] = useState([]); // all pending stock records (buys + possible others)
  const [rawSells, setRawSells] = useState([]); // server-provided sell requests
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [processingId, setProcessingId] = useState(null);
  const [confirmAction, setConfirmAction] = useState({ id: null, action: null });
  const [approvedSharesMap, setApprovedSharesMap] = useState({}); // for sell approvals
  ...
  const refresh = async () => {
    ...
    setError(null);
    try {
      const [pRes, sRes] = await Promise.all([fetch("/api/user-stocks"), fetch("/api/admin/sell-requests")]);
      const pData = await pRes.json();
      const sData = await sRes.json();
      setRawPurchases(pData.stocks || []);
      setRawSells(sData.sells || []);
  ...
  // ensure buys only show non-sell requests (status NOT containing "sell")
  const buys = useMemo(() => rawPurchases.filter((r) => !/sell/i.test(String(r.status || ""))), [rawPurchases]);
  // ensure sells only show sell requests (defensive filter)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-bl from-[#0b1221] via-[#071428] to-[#001f3f] text-white py-8">
